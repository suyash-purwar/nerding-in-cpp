...
spoon_t* get_first_spoon(const philosopher_t* philosopher) {
    return philosopher->id % 2 == 0 ? philosopher->left_spoon : philosopher->right_spoon;
}

spoon_t* get_second_spoon(const philosopher_t* philosopher) {
    return philosopher->id % 2 == 0 ? philosopher->right_spoon : philosopher->left_spoon;
}

void* philosopher_fn(void* arg) {
    philosopher_t* philosopher = arg;

    spoon_t* first_spoon = get_first_spoon(philosopher);
    spoon_t* second_spoon = get_second_spoon(philosopher);

    while (true) {
        pthread_mutex_lock(&first_spoon->mutex);
        log(philosopher, "Spoon acquired %d", first_spoon->id);

        sleep(1);

        pthread_mutex_lock(&second_spoon->mutex);
        log(philosopher, "Spoon acquired %d", second_spoon->id);

        philosopher->eat_count++;
        log(philosopher, "Eats %d times", philosopher->eat_count);

        pthread_mutex_unlock(&second_spoon->mutex);
        log(philosopher, "Spoon released %d", second_spoon->id);

        pthread_mutex_unlock(&first_spoon->mutex);
        log(philosopher, "Spoon released %d", first_spoon->id);
